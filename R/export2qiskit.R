...
#' 
#' @export
export2qiskit <- function(object, filename="circuit.py", append=FALSE) {
  if(!append) file.create(filename)
  olines <- c("# automatically generated by qsimulatR")
  if(object@circuit$ncbits == 0) {
    olines <- c(olines, paste0("qc = QuantumCircuit(", object@nbits, ")"))
  }
  else {
    olines <- c(olines, paste0("qc = QuantumCircuit(", object@nbits, ",", object@nbits, ")"))
  }
