...
implement quantum state manipulations intuitively via multiplcations
`*`. 
...
The such exported code can in principle be run on directly
on IBM's real quantum hardware.

We have developed `qsimulatR` for a lecture on _Quantum Computing_
tought at the University of Bonn, Germany. It is available on CRAN
since last week.
...
even thought that is not realistic on a real quantum device and should
...
represented by a vertical line. In `qsimulatR` a circuit can be
...
neccessary. The most important two-qubit gate is the controlled `NOT`
...
Another widely used two-qubit gate is the `SWAP` gate, which swappes
...
For more details we refer to the `qsimulatR` vignette. Based on the
...
phase estimation allows to estimate phase of eigenvalues of unitary
...
as binary fraction with a single bit exactly. Here we use $t=2$ qubits
