...
```{r}
epsilon <- 1/4
digits <- t-ceiling(log(2+1/(2*epsilon))/log(2)) # note the log in base-2
digits
```
...
s <- sin(alpha)
c <- cos(alpha)
M <- array(as.complex(c(c, -s, s, c)), dim=c(2,2)) # note that R fills the matrix columns first
...
is indeed smaller than the maximal deviation we expected.
